...
//     var colorTexture: MTLTexture?
    ...
    init?(metalKitView: MTKView, terminal: TTerminal, tty: TTY) {
        ...
        vertexFunction = library.makeFunction(name: "vertexFunction")!
        fragmentFunction = library.makeFunction(name: "fragmentFunction")!

        currentWidth = terminal.WIDTH
        currentHeight = terminal.HEIGHT
    ...
    func draw(in view: MTKView) {
        ...
        if vertices.count > 0 {
            ...
            renderEncoder.drawIndexedPrimitives(
                ...
                indexCount: currentWidth * currentHeight * 6,
    ...
    func tick() {
        ...
        vertices = []
        indices = []
        fill(&vertices, &indices, &terminal.cells, cursor: terminal.cursor, width: Int(currentWidth))
    ...
    func setupTexture() {
        ...
        vertexBuffer = device.makeBuffer(bytes: vertices, length: vertices.count * MemoryLayout<Vertex>.stride, options: MTLResourceOptions.storageModeShared)!
        indexBuffer = device.makeBuffer(bytes: indices, length: indices.count * MemoryLayout<ushort>.stride, options: MTLResourceOptions.storageModeShared)!
        ...
        for i in 32 ..< 127 {
            GenerateGlyph(texture: fontTexture!, char: Character(UnicodeScalar(i)!))
        }
    }
}
